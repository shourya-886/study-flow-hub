...
function ClassTimetable({ timetable, selectedDay, setSelectedDay, updateClass }: { timetable: Record<Day, string[]>; selectedDay: Day; setSelectedDay: (day: Day) => void; updateClass: (day: Day, index: number, value: string) => void }) {
  return <div>
    <SectionHeading eyebrow="Your week at a glance" title="Class timetable" description="Choose a day, then set the subject in each period. Every MPJ period is a shared one — 45 min of mental ability, 45 min of BIO KP — while BIO KP can also fill a full 1 hr 25 min period on its own. Set HOLIDAY in any period and the rest of that day is off: in period 01 it is a full holiday, later it becomes a half day — either way, 3 hours of study at home." />
    <div className="mt-10 grid gap-10 lg:grid-cols-[1fr_260px]">
      <div className="overflow-x-auto border-y hairline">
        <div className="min-w-[760px]">
          ...
          {periods.map((period, index) => <div key={period.label} className="grid grid-cols-[112px_repeat(5,minmax(120px,1fr))] border-b hairline last:border-b-0">
            ...
            {days.map((day) => { const off = holidayIndex(timetable[day]); const pastOff = off !== null && index > off; const isHolidayCell = off === index; return <div key={day} className={`border-l hairline p-3 ${selectedDay === day ? "bg-secondary-ground/60" : ""} ${pastOff ? "opacity-40" : ""}`}>{pastOff ? <span className="block py-2 font-mono text-xs tracking-[0.08em] text-muted">—</span> : <select aria-label={`${day} period ${period.label}`} value={timetable[day][index]} onChange={(event) => updateClass(day, index, event.target.value)} className="w-full cursor-pointer border-b border-line bg-transparent py-2 font-mono text-xs tracking-[0.08em] text-ink outline-none focus:border-pigment">{subjects.map((subject) => <option key={subject.code} value={subject.code}>{subject.code}</option>)}</select>}<span className="mt-2 block text-xs text-muted">{pastOff ? "Off — day ends early" : subjectName(timetable[day][index]!)}</span>{timetable[day][index]! === "MPJ" && !pastOff && <span className="mt-2 block border-t hairline pt-2 mono-label text-[9px] text-pigment">split · MPJ 45′ + BIO KP 45′</span>}{isHolidayCell && <span className="mt-2 block border-t hairline pt-2 mono-label text-[9px] text-pigment">{index === 0 ? "full holiday · 3h home study" : "half day from here · 3h home study"}</span>}</div>; })}
          </div>)}
      ...
      <aside className="border-t hairline pt-5 lg:border-l lg:border-t-0 lg:pl-7">
        ...
        {dayIsOff(timetable[selectedDay]) && <p className="mt-4 border-t hairline pt-4 mono-label text-[10px] text-pigment">{dayIsOff(timetable[selectedDay]) === "full" ? "Full holiday — 3 hours of study at home" : "Half day — 3 hours of study at home"}</p>}
...
function StudyPlan({ exams, plan, setActiveTab, timetable }: { exams: Exam[]; plan: { exam: Exam; topic: string; day: number }[]; setActiveTab: (tab: Tab) => void; timetable: Record<Day, string[]> }) {
  ...
  return <div><SectionHeading eyebrow="From chapters to action" title="Study plan" description="Each syllabus topic becomes a small daily lesson. Revise the last topic before the exam date." />
    {offDays.length > 0 && <div className="mt-8 border-y hairline py-6"><p className="mono-label text-pigment">Home study / 3 hours</p><div className="mt-4 space-y-0 divide-y hairline">{offDays.map((day) => <div key={day} className="flex items-center justify-between py-3"><span className="text-sm text-ink">{day}</span><span className="mono-label text-muted">{dayIsOff(timetable[day]) === "full" ? "Full holiday · 3 hours to study" : "Half day · 3 hours to study"}</span></div>)}</div></div>}
